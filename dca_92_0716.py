...
def subgradient(z: list) -> np.array:
    '''
    \sum _{i=1}^n z_i^2を点z_^(t)で微分する．
    return : np.array
    '''
    return 2 * np.array(z)
...
def dca(T: np.ndarray, C: np.ndarray, theta: np.array, r: np.array, rho: int, epsilon: float, z_t_minus_1:np.array, max_iter: int) -> float:
    ...
    2. 凸最適化問題を解く
    3. 得られたzを0-1変数に戻す
    4. 0-1変数に戻したzを使ってXを評価する
    5. 評価値が悪化していたら終了，そうでなければ1に戻る
    '''
    ...
    for iteration in range(max_iter):
        ...
        # print("f_t:", f_t)  
        # print("zの和：", np.sum(z_t))
        # X_tのサイズ
        print("X_tのサイズ：", X_t.shape)
        print("T@X_t", T @ X_t)
        print("T@X_tのサイズ", (T @ X_t).shape) 
        print("z_tの非ゼロ要素数：", np.count_nonzero(z_t))
        # f_t_c = np.sum(np.transpose(C) * X_t)
        # print("最適値：",f_t_c)

        # zを0-1変数に戻す
        z_hat = process_vector(z_t, p)

        # Xを評価する
        X, f_t_hat = eval_X(T, C, theta, r, z_hat)

        # 評価値が悪化していたら終了
        # if f_t_hat > f_t_minus_1:
        # if np.abs(f_t_minus_1 - f_t) < 0.001:
        # if f_t > f_t_minus_1:
        # if np.abs(f_t_hat - f_t_hat_minus_1)<0.001:
    ...
    # print("最適解z_hat:", process_vector(z_t_minus_1, p))
...
# DCAの実行
dca(T, C, theta, xi, 100, 0.3, z_t_minus_1, 10)