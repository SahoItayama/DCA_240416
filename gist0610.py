...
def gist(T, C, M, xi, epsilon, lambda_, max_iter, z_0):
    ...
    for iteration in range(max_iter):
        # 最適化問題を解く
        X_t, obj_val= solve_main(T, C, M, xi, epsilon, lambda_, z_t_minus_1)

        # Xの写像を計算
        X_t = X_t - 0.1 * C
        # X_t = np.array([prox_operator(X_t[k, j] - (1 / 10) * C[k, j], 1 / 10, 5) for k in range(K) for j in range(J)]).reshape(K, J)
        # X_t = np.array(X_t)
        # X_t = np.array([soft_thresholding(X_t[k, j], lambda_) for k in range(K) for j in range(J)]).reshape(K, J)
        # zの近接写像を計算
    ...
    with open('gist_time.txt', 'a') as f:
        f.write(str(J)+","+str(n)+"\n"+ str(end-start) + '\n')
    # T @ X >= (1-z[i]) * xi[i]を満たすか確認
    # for i in range(n):
    #     if np.all(T @ X_t >= (1-z_t[i]) * xi[i]):
    #         print(f"制約条件{i}を満たしています。")
    #     else:
    #         print(f"制約条件{i}を満たしていません。")
